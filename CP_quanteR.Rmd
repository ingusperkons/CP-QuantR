...
## Note
Peak integration is done in Skyline. 
...
```{r table example, echo=FALSE, warning=FALSE}
tmp.tbl.ex = data.frame("Precursor.Neutral.Formula" = as.character(c("C10Cl5H18","C10Cl6H17","C10Cl7H16")),
                        "Product.Mz" = as.numeric(c(314.98275,346.94669,382.90479)),
                        "Area" = as.numeric(c(1.2e5, 1.8e5, 1.5e5)),
                        "Replicate" = as.character(c("Spike_10ng","Spike_10ng","Spike_10ng")))
kable(tmp.tbl.ex[1:3,], caption = "Example table from Skyline .csv report")
...
### Define the volume of the final sample extract (μL)  
  
```{r final volume}
Final.extract.volume = 100 #μL
```
...
```{r write XLSX raw data, echo=FALSE}
...
```{r, echo = FALSE}
(spread(data = store.lc.1M[,-8]%>% filter(Sample.type == "Sample"),key = Sample.name, value = peak.area )%>% arrange(row.identity) %>% as.data.frame(.))[1:5,1:10] %>% kable(.)
```
...
```{r ISTD graph, echo=FALSE, warning= FALSE}
store.lc.1M %>%
...
Example for $C_nH_{2n+2-m}Cl_m$:  

${Cl\%}_{C_nH_{2n+2-m}Cl_m} = \frac{m*35.5}{n*12+2n+2-m+m*35.5}$
...
```{r XLSX export norm sub, echo=FALSE}
if (any(store.lc.1M$Sample.type == "Blank")) {
...
```{r show tbl sub, echo = FALSE}
(spread(data = store.lc.1M[,c(1:4,6:7,9:10,12)]%>% filter(Sample.type == "Sample"),key = Sample.name, value = Peak.area.ratio.bs)%>% arrange(row.identity) %>% as.data.frame())[1:5,1:10] %>% kable(.)
```

### Output in excel by sample type (normalized by internal standard, **not blank subtracted**)
  
```{r XLSX export norm non-sub, echo=FALSE}
if (any(store.lc.1M$Sample.type == "Blank")) {
...
```{r cal info show, echo=FALSE}
read_xlsx("Reference_sheet.xlsx", sheet = 2) %>%
  kable(.,caption = "Calibration data table from \"Reference_sheet.xlsx\" ")
```
...
```{r cal plot example, echo =FALSE, fig.width=10,fig.height=11}
grid.arrange(Chlorine.vs.Resp.plots[[10]], Conc.vs.Resp.plots[[10]], ncol=1, heights = c(10,20), widths = 1)
```
  
### Fetch R-squared and slope values from calibration and join with main dataset
  
```{r fetch R2 slope add}
...
The results are shown as ng per extract. If you wish to obtain final concentration in the extract, this value has to be divided by sample volume (μL)  
  
```{r export blank sub results}
rbind(Results.sheet %>%
  ungroup() %>%
...
Results.sheet %>%
  ...
  mutate(Conc.nguL.bs = Conc.nguL.bs*Final.extract.volume) %>%
  spread(key = Sample.name, value = Conc.nguL.bs) %>%
  arrange(Chain.length) %>% as.data.frame())[,1:5] %>% kable(.)
```

### Export results without blank subtraction  
The results are shown as ng per extract. If you wish to obtain final concentration in the extract, this value has to be divided by sample volume (μL)  
  
```{r export non-blank sub results}
rbind(Results.sheet %>%
        ungroup() %>%
...
```{r plot conc groups, echo=FALSE,message=FALSE}
Results.sheet %>%
  ungroup() %>%
...
```{r plot conc groups non-bs, echo=FALSE}
Results.sheet %>%
  ungroup() %>%
  filter(Sample.name %in% Sample.subset) %>%
  select(Sample.name,CP.type,Sum.CPs.nguL) %>%
...
```{r plot conc chain non-bs, echo=FALSE}
Results.sheet %>%
  ungroup() %>%
  filter(Sample.name %in% Sample.subset) %>%
  select(Sample.name,Chain.length,Conc.nguL) %>%
...
```{r hom profile single rel, echo=FALSE}
...
```{r hom profile single rel no-bs, echo=FALSE}
...
```{r hom profile single abs, echo=FALSE}
...
```{r hom profile single abs non-bs, echo=FALSE}
